...
class MDNemulator_polyfit(pl.LightningModule):
    def __init__(self, nhidden,learning_rate,epochs,ncomp, input_size, nhidden=3):
        super().__init__()
        ...
        params_encoder = np.linspace(ncomp,input_size,nhidden)
        modules = []
        for k in range(nhidden-1):
            modules.append(nn.Linear(int(params[k]) ,int(params[k+1])))
            modules.append(nn.LeakyReLU(0.2))  
    ...
    def training_step(self, batch, batch_idx):
        ...
        xlatent,xout  = self(x)#.cuda()
        loss = self.loss()(xout,xinp)
